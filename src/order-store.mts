...
import type { connectionConfig, tableConfigYAML } from './definition.mjs';
...
const importLockState = new AsyncLocalStorage<{ error?: Error }>();
export function assertOrderImportLock(): void {
    const error = importLockState.getStore()?.error;
    if (error) throw new Error('PostgreSQL import lock connection was lost; refusing publication', { cause: error });
}

export async function withOrderImportLock<T>(config: connectionConfig, action: () => Promise<T>): Promise<T> {
    if (config.technology !== 'postgres') return action();
    ...
    try {
        ...
        if (!result.rows[0].acquired) throw new Error('Another loader or order backfill is running');
        let checking = false;
        heartbeat = setInterval(async () => {
            if (checking || state.error) return;
            checking = true;
...
export async function checkOrderSchema(client: Pick<PoolClient, 'query'>, company: string): Promise<void> {
    if (!company) throw new Error('Order import requires an explicit Tally company');
    const source = await client.query("select value from public.config where name='Company Name'");
    if (source.rows.length > 1 || (source.rows.length && source.rows[0].value !== company)) throw new Error('Tally company does not match PostgreSQL');
    if (!source.rows.length) {
        const existing = await client.query('select exists(select 1 from public.trn_voucher) as populated');
        if (existing.rows[0].populated) throw new Error('Company metadata is missing from a populated voucher mirror');
    }
...
export interface RemovedHeader { guid: string; alterid: number }

export async function findRemovedOrderHeaders(client: Pick<PoolClient, 'query'>, checkpoint: number): Promise<RemovedHeader[]> {
    if (!Number.isSafeInteger(checkpoint) || checkpoint < 0) throw new Error('Invalid voucher diff checkpoint');
    ...
    if (invalid.rowCount) throw new Error('Invalid, duplicate, or backwards voucher diff identity');
    const result = await client.query<RemovedHeader>(`select v.guid,v.alterid from public.trn_voucher v
...
export async function publishVoucherHeaders(client: PoolClient, table: tableConfigYAML,
    data: string, removed: RemovedHeader[] = [], bounds?: { after: number; through: number }): Promise<void> {
    if (bounds && (!Number.isSafeInteger(bounds.after) || !Number.isSafeInteger(bounds.through)
        || bounds.after < 0 || bounds.through < bounds.after)) throw new Error('Invalid voucher AlterID window');
    ...
    if (header !== table.fields.map(field => field.name).join('\t')) throw new Error('Voucher data columns differ from definition');
    const rows = lines.filter(Boolean).map(line => {
        const values: (string | null)[] = line.split('\t');
        if (values.length !== table.fields.length) throw new Error('Voucher data column count mismatch');
        ...
        if (values[names.indexOf('order_number')] !== (number || '')) throw new Error('Inconsistent voucher order number');
    ...
    try {
        ...
        for (const values of rows) {
            ...
            if (previous.rows.some(row => row.alterid > revision)) throw new Error('Refusing an older voucher header');
...
export async function applyOrderBackfill(client: Pick<PoolClient, 'query'>, rows: OrderVoucher[], company: string, companyGuid?: string) {
    ...
    for (const row of rows) {
        ...
        if (!row.guid || seen.has(row.guid) || !Number.isSafeInteger(row.alterid) || row.alterid < 0
            || resolveOrderNumber(row.order_details) !== row.order_number) throw new Error('Invalid backfill voucher');
        seen.add(row.guid);
    }
    let updated = 0;
    ...
    try {
        ...
        for (const row of rows) {
            ...
            const result = await client.query(`update public.trn_voucher
                set order_details=$1::jsonb,order_number=$2 where guid=$3 and alterid=$4`,
            [JSON.stringify(row.order_details), row.order_number, row.guid, row.alterid]);
            if (result.rowCount === 1) updated++;
            else if (result.rowCount === 0) skipped.push(row.guid);
            else throw new Error('Duplicate voucher GUID during backfill');
        }
        assertOrderImportLock();
        await client.query('commit');

...
export const DIAGNOSTIC_CASES = ['company', 'guid', 'direct', 'filters', 'fields', 'orders', 'count'] as const;
...
export function diagnosticRequest(config: tallyConfig, table: tableConfigYAML, options: Options, selected: Case): string {
    ...
    if (selected === 'company') definition = { name: 'company', nature: 'Primary', collection: 'Company', fetch: ['Name'],
...
export function inspectDiagnosticResponse(body: string, selected: Case, guid: string, company: string) {
...
export async function diagnoseVoucher(config: tallyConfig, options: Options) {
    ...
    if (options.case === 'direct') diagnosticRequest(config, table, options, 'direct');

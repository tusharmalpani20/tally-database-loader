...
export class BackfillDiagnostics {
    ...
    constructor(private options: BackfillLogOptions = {}) {}
...
export async function fetchBackfillVouchers(transport: TallyTransport, request: string,
    ...
    heartbeat.unref();
    let response: string;
    try { response = await transport.post(request); }
    ...
    for (const match of response.matchAll(/<([A-Za-z_][\w.:-]*)(?=[\s/>])/g)) {
        ...
        if (names.size < 40) names.add(name);
        if (name === 'KEEXPORTCOUNT') count = true;
        if (name === 'KECOMPANY') companyTag = true;
        if (name === 'KEVOUCHER') vouchers++;

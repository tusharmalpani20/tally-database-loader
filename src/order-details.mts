...
export interface OrderDetail { order_number: string; order_date: string | null }
...
export function normalizeOrderNumber(value: string, guid: string, entryIndex: number, context: Record<string, unknown> = {}): string {
...
export function resolveOrderNumber(details: OrderDetail[]): string | null {
...
export function validateOrderDetails(value: unknown): asserts value is OrderDetail[] {
    if (!Array.isArray(value)) throw new Error('Order details must be an extracted list');
    for (const entry of value) {
        ...
        if (entry.order_date !== null && (typeof entry.order_date !== 'string'
            ...
            || orderDate(entry.order_date) !== entry.order_date)) throw new Error('Invalid voucher order date');
    }
}

function orderDate(value: string): string | null {
...
function scalar(object: Record<string, unknown>, name: string, trim = true): string {
    if (!(name in object) || typeof object[name] !== 'string') {
        throw new Error(`Missing or non-scalar order export field ${name}`);
    }
    return trim ? (object[name] as string).trim() : object[name] as string;
}
...
function countedVoucherRows(xml: string, expectedCompany?: string): Record<string, unknown>[] {
    ...
    if (!Array.isArray(rows) || rows.length !== Number(count)) throw new Error('Voucher export count mismatch');
    return rows;
}
...
export function parseVoucherIdentities(xml: string, expectedCompany: string): string[][] {
    ...
    return countedVoucherRows(xml, expectedCompany).map(row => {
        ...
        if (!guid || seen.has(guid) || /[\t\r\n]/.test(guid) || !/^\d+$/.test(alterid)
            || !Number.isSafeInteger(Number(alterid))) throw new Error('Invalid voucher diff identity');
        seen.add(guid);
        return [guid, alterid];
    });
}

export function parseOrderVouchers(xml: string, table: tableConfigYAML, expectedCompany?: string, direct = false): OrderVoucher[] {
    ...
    return rows.map((row: Record<string, unknown>) => {
        ...
        if (direct && scalar(row, 'KEDIRECTELIGIBLE') !== '1') throw new Error('Direct voucher is outside the eligible filters or export period');
        ...
        const details: OrderDetail[] = orders.map((entry, index) => {
            try {
                ...
                return { order_number: normalizeOrderNumber(scalar(entry, 'KEORDERNUMBER', false), guid, index,
                    { ...context, company: expectedCompany, orderDate: order_date }), order_date };
            } catch (error) {
...
export function addOrderDetailReport(xml: string, table: tableConfigYAML): string {
    if (table.voucher_identities) {
        if (table.fields.map(field => field.name).join(',') !== 'guid,alterid') throw new Error('Invalid voucher diff definition');
...
export function directOrderReport(xml: string, masterId: string): string {
    if (!/^[1-9]\d{0,9}$/.test(masterId)) throw new Error('Invalid Tally MasterID (not AlterID)');
    const collections = xml.match(/<COLLECTION NAME="MyCollection">[\s\S]*?<\/COLLECTION>/g);
    if (collections?.length !== 1) throw new Error('Direct backfill requires exactly one voucher collection');
    ...
    if (filterTags.length !== 1) throw new Error('Direct backfill requires voucher eligibility filters');

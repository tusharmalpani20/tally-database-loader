...
export class HttpTallyTransport {
    ...
    metrics;
    diagnostics;
    constructor(config, metrics, diagnostics = {}) {
        this.config = config;
        this.metrics = metrics;
        this.diagnostics = diagnostics;
        if (diagnostics.timeoutMs !== undefined && (!Number.isSafeInteger(diagnostics.timeoutMs) || diagnostics.timeoutMs <= 0 || diagnostics.timeoutMs > 2147483647))
    ...
    reportDiagnostic(action) {
        ...
        try {
            ...
            console.error('Unable to write Tally transport diagnostic output.');
        }
    }
    /**
    ...
    async post(msg) {
        ...
        const queueTimer = this.diagnostics.progress ? setInterval(() => this.reportDiagnostic(() => this.diagnostics.progress?.({ phase: 'lock_wait', elapsedMs: Date.now() - startedAt })), 15000) : undefined;
        queueTimer?.unref();
        try {
            const response = await withTallyLock(this.config.server, this.config.port, 'tally export', () => this.postUnlocked(msg), waitMs => { if (queueTimer)
                clearInterval(queueTimer); lockWaitMs = waitMs; this.reportDiagnostic(() => this.diagnostics.progress?.({ phase: 'lock_acquired', elapsedMs: waitMs })); });

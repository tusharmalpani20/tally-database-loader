...
import { validateDirectOrderScope } from './direct-order-protocol.mjs';
...
export function batchOrderProbe(scope, source, empty) {
    validateDirectOrderScope(scope);
    if (!source.order_details || source.name !== 'trn_voucher' || !source.filters?.length)
        throw new Error('Batch probes require the order-detail voucher profile with eligibility filters');
    ...
    const replace = (anchor, value) => {
        if (xml.split(anchor).length !== 2)
            throw new Error('Batch probe template is ambiguous');
...
export function inspectBatchOrderProbe(xml, scope, empty) {
    validateDirectOrderScope(scope);
    if (/<!DOCTYPE|<!ENTITY/i.test(xml) || XMLValidator.validate(xml) !== true)
        throw new Error('Invalid batch probe XML');
    ...
    const shape = (value, names) => {
        if (!value || typeof value !== 'object' || Array.isArray(value) || Object.keys(value).some(key => !names.includes(key)))
            throw new Error('Unexpected batch probe structure');
    };
    shape(parsed, ['ENVELOPE']);
    shape(parsed.ENVELOPE, ['KEBATCH']);
    ...
    if (batch['@_PROTOCOL'] !== 'KE_BATCH_PROBE_V1' || batch['@_COMPANY'] !== scope.company
        ...
        || batch['@_COUNT'] !== (empty ? '0' : '1'))
        throw new Error('Missing or incorrect batch probe metadata');
    const rows = batch.KEVOUCHER ?? [];
    if (!Array.isArray(rows) || rows.length !== (empty ? 0 : 1))
        throw new Error('Batch probe count mismatch');
    for (const row of rows) {
        shape(row, ['F01', 'F02', 'FLDBLANK']);
        if (row.F01 !== scope.guid || typeof row.F02 !== 'string' || !/^\d+$/.test(row.F02)
            || !Number.isSafeInteger(Number(row.F02)))
            throw new Error('Invalid batch probe identity');
    }
}
//# sourceMappingURL=batch-order-probe.mjs.map